...
import { provinceSihanoukville } from '../data';
import { useNavigate, useParams } from 'react-router-dom';

const SihanoukvilleDetail = () => {
  const { id } = useParams();
    const navigate = useNavigate();

    // Find the selected place info
    const result =provinceSihanoukville.find((p) => p.id === parseInt(id));
  ...
  return (
     <div className="font-sans bg-gray-50 text-gray-800">
      ...
      <div className="max-w-6xl mx-auto px-6 py-16">
        <h2 className="text-3xl font-bold text-yellow-700 mb-6 border-l-4 border-yellow-700 pl-3">
          About {result.name}
        </h2>
      ...
      <div className="bg-gradient-to-b from-emerald-100 via-green-50 to-white text-center py-20">
        ...
        <Link to="/contect">
          <button className="px-8 py-3 bg-white text-yellow-700 font-semibold rounded-lg shadow hover:bg-gray-200 transition">
            Plan Your Trip
          </button>
        </Link>
      </div>
...
export default SihanoukvilleDetail
