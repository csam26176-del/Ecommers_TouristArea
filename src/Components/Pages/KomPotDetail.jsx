...
import { provinceKomPot } from '../data';

export const KomPotDetail = () => {
    const { id } = useParams();
    const navigate = useNavigate();

    // Find the selected place info
    const result =provinceKomPot.find((p) => p.id === parseInt(id));
  ...
  return (
      <div className="font-sans bg-gray-50 text-gray-800">
        ...
        <div className="relative h-[40vh] md:h-[45vh] lg:h-[80vh]">
          ...
          <button
            onClick={() => navigate(-1)}
            className="absolute bottom-6 left-6 flex items-center gap-2 px-4 md:py-2 py-1 bg-gray-200 bg-opacity-80 text-gray-800 rounded-lg shadow hover:bg-gray-300 transition z-10"
          >
          ...
          <div className="absolute inset-0 bg-black/50 flex flex-col justify-center items-center text-center text-white px-4">
            <span className="bg-red-600 px-4 py-1 rounded-full text-xs sm:text-sm uppercase tracking-wider md:mb-5 mb-2 shadow-md">
              {result.category}
            </span>
            <h1 className="text-3xl sm:text-5xl font-bold md:mb-5 mb-2 drop-shadow-lg">
              {result.name}
            </h1>
            <p className="max-w-2xl text-gray-200 text-sm sm:text-base leading-relaxed mb-8">
              {result.description}
            </p>
          </div>
        </div>
        ...
        {result.gallery && result.gallery.length > 0 && (
          <div className="bg-white py-10">
            <h2 className="text-3xl md:text-3xl font-bold text-center text-yellow-700 mb-4">
              Gallery
            </h2>
              ...
              {result.gallery.map((img, i) => (
                ...
                >
                  <img
                    src={img}
                    alt={`${result.name} ${i + 1}`}
                    className="w-full h-[220px] md:h-[200px] lg:h-[250px] object-cover object-center hover:scale-110 transition-transform duration-500"
                  />
                </div>
              ))}
            </div>
        ...
        <div className="max-w-6xl mx-auto px-6 md:mt-10 mt-4">
          <h2 className="text-3xl font-bold text-yellow-700 mb-12 text-center">
            Location
          </h2>
        ...
        <div className="max-w-6xl mx-auto px-6 py-12">
          <div className="flex flex-wrap justify-center gap-6 text-center">
            {[
              ...
              {
                title: 'Address',
                detail: result.address || `${result.name}, Kep, Cambodia`,
              },
            ].map((item, i) => (
              <div
                key={i}
        ...
        {/* 📩 CTA Section */}
        <div className="bg-gradient-to-b from-emerald-100 via-green-50 to-white text-center py-10 md:py-20 h-[30vh] md:h-[28vh] lg:h-[40vh]">
